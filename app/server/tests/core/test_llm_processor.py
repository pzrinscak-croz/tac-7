...
class TestLLMProcessor:
    ...
    def test_format_previous_context_none(self):
        # No context when either value is missing
        assert format_previous_context(None, None) == ""
        assert format_previous_context("show all users", None) == ""
        assert format_previous_context(None, "SELECT * FROM users") == ""

    def test_format_previous_context_with_values(self):
        # Context block includes both previous question and previous SQL
    ...
    def test_generate_sql_with_openai_includes_context(self, mock_openai_class):
        # When context is provided, the prompt contains the previous question and SQL
        mock_client = MagicMock()
        mock_openai_class.return_value = mock_client
    ...
    @patch('core.llm_processor.OpenAI')
    def test_generate_sql_with_openai_no_context(self, mock_openai_class):
        # Backward compatible: standalone query has no context block in the prompt
        mock_client = MagicMock()
        mock_openai_class.return_value = mock_client
    ...
    @patch('core.llm_processor.generate_sql_with_openai')
    def test_generate_sql_forwards_context(self, mock_openai_func):
        # generate_sql forwards previous_query/previous_sql from the request
    ...
    def test_query_request_backward_compatible(self):
        # A request without the new fields validates and defaults context to None
        request = QueryRequest(query="show all users", llm_provider="openai")
        assert request.previous_query is None
        assert request.previous_sql is None